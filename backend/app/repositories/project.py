...
class ProjectRepository:
    ...
    async def get_by_repo(self, owner: str, name: str) -> Project | None:
        return await self.db.scalar(
            select(Project).where(Project.repo_owner == owner, Project.repo_name == name)
        )
    ...
    async def get_active_invitation(self, project_id: uuid.UUID, now: datetime) -> InvitationLink | None:
        return await self.db.scalar(
            select(InvitationLink)
            .where(InvitationLink.project_id == project_id, InvitationLink.expires_at > now)
