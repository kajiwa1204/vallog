...
class GitHubPullRequest(Base):
    ...
    gh_created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    merged_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    closed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    # issues/events の reopened を集計。品質・可用性カテゴリの手戻り率指標に使う
...
class GitHubIssueAssignee(Base):
    ...
    issue_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("github_issues.id", ondelete="CASCADE"), nullable=False
    )
    login: Mapped[str] = mapped_column(String, nullable=False)
    # issues/events の assigned イベントから取得。タスク完了スピードの計測起点
...
class GitHubReview(Base):
    ...
    # レビューに紐づくインラインコメント数（/pulls/comments を pull_request_review_id で集計）
