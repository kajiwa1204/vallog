...
async def generate_summary(
    ...
    active = await job_repo.get_active(project.id, login)
    if active is not None:
        return JSONResponse(
            status_code=200,
            content=SummaryJobResponse.model_validate(active).model_dump(mode="json"),
        )

    job = await job_repo.create(project.id, login)
    await db.commit()

    summary_service._launch_summary_job(job.id, project.id, user.id, login)
...
async def generate_pr_summary(
    ...
    active = await job_repo.get_active(project.id, login, pr_number)
    if active is not None:
        return JSONResponse(
            status_code=200,
            content=SummaryJobResponse.model_validate(active).model_dump(mode="json"),
    ...
    job = await job_repo.create(project.id, login, pr_number)
    await db.commit()

    summary_service._launch_summary_job(job.id, project.id, user.id, login, pr_number)
...
async def list_pr_summaries(login: str, project: MemberProject, db: DB):
    ...
    items = [
        PRSummaryItem(
            ...
            content=summaries_by_number[pr.number].content if pr.number in summaries_by_number else None,
            generated_at=summaries_by_number[pr.number].generated_at if pr.number in summaries_by_number else None,
            job=SummaryJobResponse.model_validate(jobs_by_pr[pr.number]) if pr.number in jobs_by_pr else None,
        )
