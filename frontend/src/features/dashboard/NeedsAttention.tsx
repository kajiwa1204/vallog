  ...
  const groups: Group[] =
    me === null
      ...
      : [
          ...
          // 自分の未承認PRは別に置く。「あなたの番」に混ぜると、レビュアーが着手して
          // いないだけの場合まで自分の停滞として突きつけることになる。
          //
          // 「待っています」ではなく「止まっています」なのは、誰の番かを判定できないため。
          // ここにはコメントだけ付いたPRも入る（承認状態が動いていないもの全部）。
          // その場合、待っているのはレビュアーかもしれないし、質問への回答かもしれない。
          // 「待っています」は相手がいることを含意するが、「止まっています」は事実だけを言う
          {
            key: "waiting",
            heading: "あなたのPRが止まっています",
            rows: rows.filter((row) => row.mine && row.kind === "review"),
          },
          {
            key: "team",
