 ...
 * スコア（画面7の補助情報）。
 *
 * **この画面で唯一スコアが出る場所であり、意図的に下部に小さく置いてある**
 * （docs/scoring_design.md「数字の降格」）。主役は上の変化ログで、スコアは案を作った
 * ときの出発点を説明するだけ。分配額はここから自動では決まらない。
 *
      ...
      {state.kind === "undisclosed" && (
        <div className={styles.undisclosed}>
          ...
          <p className={styles.undisclosedBody}>
            ...
            {selectedIsFinalized &&
              "確定した配分は上の表に残っています。もう一度検討するときは新しい案を作成してください。"}
          </p>
        </div>
      )}
